...
export function registerWebhook(program: Command): void {
  program
    ...
    .action((opts: { filter?: string; url: string }) => {
      ...
      if (isAgentMode()) {
        console.log(JSON.stringify({ meta: { command: "webhook:create", wf_version: "3.1.3" }, webhook: hook }));
      } else {
  ...
  program
    ...
    .action(() => {
      ...
      if (isAgentMode()) {
        console.log(JSON.stringify({ meta: { command: "webhook:list", wf_version: "3.1.3" }, webhooks: hooks, count: hooks.length }));
        return;
      }
  ...
  program
    ...
    .action((id: string) => {
      ...
      if (isAgentMode()) {
        console.log(JSON.stringify({ meta: { command: "webhook:delete", wf_version: "3.1.3" }, deleted: id }));
      } else {
  ...
  program
    ...
    .action(async (id: string) => {
      ...
      try {
        ...
        if (isAgentMode()) {
          console.log(JSON.stringify({ meta: { command: "webhook:test", wf_version: "3.1.3" }, status: res.status, ok: res.ok }));
        } else {
          if (res.ok) {
