...
export function registerConfigCommands(program: Command): void {
    ...
    .action((key: string) => {
      ...
      if (isAgentMode()) {
        console.log(JSON.stringify({
          meta: { command: "config:get", wf_version: "3.0.1" },
          key,
          value: value ?? null,
        }));
  ...
  program
    ...
    .action((key: string, value: string) => {
      ...
      if (isAgentMode()) {
        console.log(JSON.stringify({
          meta: { command: "config:set", wf_version: "3.0.1" },
          key,
          value,
          status: "ok",
        }));
  ...
  aliasCmd
    ...
    .action((name: string, expansion: string) => {
      ...
      if (isAgentMode()) {
        console.log(JSON.stringify({
          meta: { command: "config:alias set", wf_version: "3.0.1" },
          alias: name,
          expansion,
          status: "ok",
        }));
  ...
  aliasCmd
    ...
    .action(() => {
      ...
      if (isAgentMode()) {
        console.log(JSON.stringify({
          meta: { command: "config:alias list", wf_version: "3.0.1" },
          aliases,
        }));
        return;
      }
  ...
  aliasCmd
    ...
    .action((name: string) => {
      ...
      if (isAgentMode()) {
        console.log(JSON.stringify({
          meta: { command: "config:alias remove", wf_version: "3.0.1" },
          alias: name,
          status: "removed",
        }));
      } else {
