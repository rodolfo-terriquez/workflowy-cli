...
export function registerWatch(program: Command): void {
  ...
  program
    ...
    .action(() => {
      if (!existsSync(PID_PATH)) {
        if (isAgentMode()) {
          console.log(JSON.stringify({ meta: { command: "watch:status", wf_version: "3.1.5" }, running: false }));
        } else {
      ...
      if (isAgentMode()) {
        console.log(JSON.stringify({ meta: { command: "watch:status", wf_version: "3.1.5" }, running: true, pid }));
      } else {
