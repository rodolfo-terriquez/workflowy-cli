...
export function registerWatch(program: Command): void {
  ...
  program
    ...
    .action(() => {
      if (!existsSync(PID_PATH)) {
        if (isAgentMode()) {
          console.log(JSON.stringify({ meta: { command: "watch:status", wf_version: "3.0.4" }, running: false }));
        } else {
      ...
      if (isAgentMode()) {
        console.log(JSON.stringify({ meta: { command: "watch:status", wf_version: "3.0.4" }, running: true, pid }));
      } else {
