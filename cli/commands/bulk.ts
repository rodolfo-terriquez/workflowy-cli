...
async function executeBulk(
  ...
  if (nodes.length === 0) {
    if (isAgentMode()) {
      console.log(JSON.stringify({ meta: { command: `node:bulk ${operation}`, wf_version: "3.1.9" }, message: "No matching nodes." }, null, 2));
    } else {
  ...
  if (opts.dryRun) {
    if (useJson) {
      console.log(JSON.stringify({
        meta: { command: `node:bulk ${operation}`, dry_run: true, wf_version: "3.1.9" },
  ...
  if (useJson) {
    ...
    console.log(JSON.stringify({
      meta: { command: `node:bulk ${operation}`, timestamp: new Date().toISOString(), account: config.activeAccount, wf_version: "3.1.9" },
