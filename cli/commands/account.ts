...
export function registerAccountCommands(program: Command): void {
    ...
    .action(() => {
      ...
      if (isAgentMode()) {
        console.log(JSON.stringify({
          meta: { command: "account:list", wf_version: "3.1.8" },
  ...
  program
    ...
    .action((name: string) => {
      ...
      if (isAgentMode()) {
        console.log(JSON.stringify({ meta: { command: "account:switch", wf_version: "3.1.8" }, active_account: name }));
      } else {
  ...
  program
    ...
    .action(() => {
      ...
      if (isAgentMode()) {
        console.log(JSON.stringify({
          meta: { command: "account:current", wf_version: "3.1.8" },
          active_account: config.activeAccount,
