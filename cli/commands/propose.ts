...
export function registerAiCommands(program: Command): void {
  ...
  program
    ...
    .action((id: string | undefined, opts: { format?: string }) => {
      ...
      if (useJson) {
        console.log(JSON.stringify({
          meta: { command: "ai:preview", timestamp: new Date().toISOString(), account: config.activeAccount, wf_version: "3.0.7" },
          proposal: {
            id: proposal.id,
  ...
  program
    ...
    .action((id: string | undefined, opts: { format?: string; all?: boolean }) => {
      ...
      if (opts.all) {
        ...
        if (isAgentMode()) {
          console.log(JSON.stringify({
            meta: { command: "ai:reject", timestamp: new Date().toISOString(), account: config.activeAccount, wf_version: "3.0.7" },
