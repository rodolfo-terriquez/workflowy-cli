  ...
  program
    ...
    .action(() => {
      ...
      if (isAgentMode()) {
        ...
        console.log(JSON.stringify({ meta: { command: "workflow:list", wf_version: "3.1.0" }, workflows }, null, 2));
        return;
      }
...
`;
      ...
      if (isAgentMode()) {
        console.log(JSON.stringify({ meta: { command: "workflow:create", wf_version: "3.1.0" }, path: filePath }));
      } else {
    ...
    .action(async (name: string, opts: { format?: string }) => {
      ...
      if (isAgentMode() || opts.format === "json") {
        console.log(JSON.stringify({
          meta: { command: "workflow:run", workflow: name, wf_version: "3.1.0" },
          results,
        }, null, 2));
      } else {
        console.log("");
