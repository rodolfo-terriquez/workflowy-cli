  ...
  if (!silent) {
    if (isAgentMode()) {
      console.log(JSON.stringify({
        meta: { command: "cache:sync", timestamp: new Date().toISOString(), wf_version: "3.0.11" },
...
function showStatus(): void {
  ...
  if (isAgentMode()) {
    console.log(JSON.stringify({
      meta: { command: "cache:sync", mode: "status", timestamp: new Date().toISOString(), wf_version: "3.0.11" },
